...
export const paymentsRouter = router({
	list: protectedProcedure
		...
		.query(async ({ input }) => {
			...
			if (input?.sponsorId)
				filters.push(eq(payment.sponsorId, input.sponsorId));
			if (input?.childId) filters.push(eq(payment.childId, input.childId));
			if (input?.paymentStatus)
				filters.push(eq(payment.paymentStatus, input.paymentStatus));
	...
	setFinancial: protectedProcedure
		.input(
			z.object({
				id: z.string(),
				status: z.enum(["sent", "confirmed", "rejected"]),
			}),
		)
		.mutation(async ({ ctx, input }) => {
			const [old] = await db
