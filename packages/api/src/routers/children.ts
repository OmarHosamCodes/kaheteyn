...
export const childrenRouter = router({
	list: protectedProcedure
		...
		.query(async ({ input }) => {
			...
			if (input?.schoolStage)
				filters.push(eq(child.schoolStage, input.schoolStage));
			if (input?.sponsorId) filters.push(eq(child.sponsorId, input.sponsorId));
			if (input?.search) {
				const q = `%${input.search}%`;
				filters.push(
					or(
						like(child.fullName, q),
						like(child.id, q),
						like(child.guardianName, q),
					) as never,
				);
			}
			const rows = await db
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (input.sponsorId) {
				const [s] = await db
					.select()
					.from(sponsor)
					.where(eq(sponsor.id, input.sponsorId));
				if (!s)
					throw new TRPCError({
						code: "BAD_REQUEST",
						message: "الكفيل غير موجود",
					});
			}
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (input.data.sponsorId) {
				const [s] = await db
					.select()
					.from(sponsor)
					.where(eq(sponsor.id, input.data.sponsorId));
				if (!s)
					throw new TRPCError({
						code: "BAD_REQUEST",
						message: "الكفيل غير موجود",
	...
	restore: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			const [exists] = await db
				.select()
				.from(child)
				.where(eq(child.id, input.data.id));
			if (exists) {
				await db
					.update(child)
					.set(input.data)
