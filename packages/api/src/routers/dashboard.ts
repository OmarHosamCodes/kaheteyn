...
export const dashboardRouter = router({
	summary: protectedProcedure.query(async () => {
		...
		const awaitingReceipts = allPayments.filter(
			(p) => !p.acknowledgmentReceipt || !p.transferReceipt,
		).length;
	...
	latestReceipts: protectedProcedure.query(async () => {
		const rows = await db
			...
			.leftJoin(child, eq(child.id, payment.childId))
			.where(
				sql`${payment.acknowledgmentReceipt} IS NOT NULL OR ${payment.transferReceipt} IS NOT NULL`,
			)
			.orderBy(desc(payment.dateSent))
			.limit(8);
		return rows.map((r) => ({
	...
	followUp: protectedProcedure.query(async () => {
		...
		const allPayments = await db.select().from(payment);
		const paymentsWithoutReceipts = allPayments.filter(
			(p) => !p.acknowledgmentReceipt || !p.transferReceipt,
		);
	...
	monthlyReport: protectedProcedure
		...
		.query(async ({ input }) => {
			...
			const items = rows.map((r) => ({
				...
				sponsorName: r.sponsorName ?? "—",
				hasReceipt:
					!!r.payment.acknowledgmentReceipt && !!r.payment.transferReceipt,
			}));
