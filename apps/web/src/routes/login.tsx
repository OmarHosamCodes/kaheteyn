...
function LoginPage() {
	...
	const signIn = useMutation({
		mutationFn: async () => {
			...
			if (result.error)
				throw new Error(result.error.message ?? "تعذّر تسجيل الدخول");
			return result.data;
		},
		onSuccess: () => {
	...
	return (
		<div className="grid min-h-svh place-items-center bg-gradient-to-br from-background via-primary/5 to-primary/15 px-4">
			<div className="w-full max-w-sm rounded-xl border bg-card p-8 shadow-sm">
				<div className="mb-6 flex justify-center">
					<Logo />
				</div>
				<h1 className="mb-1 text-center font-bold text-xl">
					تسجيل دخول المدير
				</h1>
				<p className="mb-6 text-center text-muted-foreground text-sm">
					نظام إدارة كفالة الأيتام
				</p>
				<p className="mb-6 px-4 text-center text-muted-foreground text-xs italic">
