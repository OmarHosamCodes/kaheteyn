...
function AdminsPage() {
	...
	const invalidate = () =>
		qc.invalidateQueries({ queryKey: trpc.admins.list.queryKey() });
	...
	const submitCreate = () => {
		...
		if (createForm.username.trim().length < 3)
			return toast.error("اسم المستخدم يجب أن يكون 3 أحرف على الأقل");
		...
		<div className="space-y-6">
			<PageHeader
				...
				actions={
					...
					>
						<PlusIcon className="ms-1 size-4" /> إضافة مشرف
					</Button>
				}
			/>
								...
								{admins.map((admin) => (
									<TableRow key={admin.id}>
										...
										<TableCell className="text-end">
											...
											>
												<KeyRoundIcon className="ms-1 size-4" /> تغيير كلمة
												المرور
											</Button>
										</TableCell>
									</TableRow>
						...
						<Field label="كلمة المرور" required>
							<div className="relative">
								...
								<button
									...
									tabIndex={-1}
									aria-label={
										showCreatePw ? "إخفاء كلمة المرور" : "إظهار كلمة المرور"
									}
								>
									{showCreatePw ? (
						...
						<Field label="كلمة المرور الجديدة" required>
							<div className="relative">
								...
								<button
									...
									tabIndex={-1}
									aria-label={
										showSetPw ? "إخفاء كلمة المرور" : "إظهار كلمة المرور"
									}
								>
									{showSetPw ? (
										<EyeOffIcon className="size-4" />
