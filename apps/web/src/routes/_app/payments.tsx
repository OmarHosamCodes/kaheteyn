...
function PaymentsPage() {
	...
	const submit = () => {
		...
		if (!/^\d{4}-\d{2}$/.test(form.monthKey))
			return toast.error("الشهر غير صالح");
		...
		<div className="space-y-6">
			...
			<Card>
				<CardContent className="grid gap-3 pt-6 md:grid-cols-5">
					<Select
						...
						onChange={(e) => setFilter({ ...filter, monthKey: e.target.value })}
					>
			...
			<Dialog open={open} onOpenChange={setOpen}>
				<DialogContent className="max-w-xl">
					<DialogHeader>
						<DialogTitle>{editingId ? "تعديل دفعة" : "دفعة جديدة"}</DialogTitle>
					</DialogHeader>
					<div className="grid gap-4">
						...
						<Field label="وصل التحويل (صورة)">
							<FileUpload
								...
								onChange={(v) => setForm({ ...form, transferReceipt: v ?? "" })}
								accept="image/*"
